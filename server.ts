  ...
  app.use(async (ctx, next) => {
    ...
    if (!value || value.operationName === "IntrospectionQuery") {
      ...
      if (noAuthResolvers.some((elem) => value.query.includes(elem))) {
        ...
        const tipo = ctx.request.headers.get("tipo") || "none";
        console.log(token);
        const user = await db
        ...
        if (user) {
          ...
          let resolvers = [""];
          if (tipo == "admin") resolvers = ["getCoches","getClients","getConductores","getViajes"];
          if (tipo == "cliente") resolvers = ["startViaje"]
          if(tipo == "conductor") resolvers = ["getStatus", "changeStatus"];
